...
void Optimize(SSData *Data, cdouble Omega0)
{
  ...
  if (MyOpt==0)
   { 
     ...
     double dx[2]={0.1*real(Omega0),0.1*imag(Omega0) };
     nlopt_set_initial_step(MyOpt, dx);

     double Lower[2]={0.5*real(Omega0), -5.0*imag(Omega0)};
     double Upper[2]={2.0*real(Omega0), +5.0*imag(Omega0)};
  ...
  double fFinal=fInitial, RCFinal=RCInitial;
  if (Data->DoOptimization)
   { double fBest;
     nlopt_optimize(MyOpt, x, &fBest);
     fFinal=Objective(2, x, 0, Data);
     RCFinal=Data->RCond;
     printf("(%.2e,%.2e; %.2e) --> (%.6e, %.6e; %.2e)\n",
             real(Omega0), imag(Omega0), fInitial, x[0], x[1], fBest);
   }
  else
   printf("w=(%.2e,%.2e) f=%e  RC=%e\n", real(Omega0), imag(Omega0), fInitial, RCInitial);
...
//
     ...
     {"Optimize",     PA_BOOL,    0, 1, (void *)&DoOptimization,     0,   ""},
...
//
  ...
  Data->DoOptimization = DoOptimization;
