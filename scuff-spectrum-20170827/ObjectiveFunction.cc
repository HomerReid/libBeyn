   ...
   for(int nsb=0; nsb<NS; nsb++)
    { int RowOffset=G->BFIndexOffset[nsa];
      ...
      G->AssembleBEMMatrixBlock(nsa, nsb, Omega, kBloch,
                                ...
                                Accelerators ? Accelerators[nsa*NS + nsb] : 0);
    };
}
...
#define NUMSTATS  5
   ...
   /***************************************************************/
   if (Data->G->LDim==0)
    Log("Assembling M at w=%s",z2s(Omega));
   else if (Data->G->LDim==1)
    Log("Assembling M at {w,k}={%s,%e}",z2s(Omega),kBloch[0]);
   else if (Data->G->LDim==2)
    Log("Assembling M at {w,k}={%s,%e,%e}",z2s(Omega),kBloch[0],kBloch[1]);
   Tic();
   ...
   for(int d=0; d<Data->G->LDim; d++)
    fprintf(ff,"%e ",kBloch[d]);
   if (DoRCond)
